...
// Функция для удаления всех строк таблицы, кроме заголовка
 ...
 rowData.shift();
  ...
  rowData.sort((first, second) => {
    for (let i = 0; i < sortArr.length; i++) {
      ...
      // Сравниваем строки в зависимости от порядка сортировки
      if (first.cells[key].innerHTML.toLowerCase() < second.cells[key].innerHTML.toLowerCase()) {
        return order ? 1 : -1;
      } else if (first.cells[key].innerHTML.toLowerCase() > second.cells[key].innerHTML.toLowerCase()) {
        return order ? -1 : 1;
      }
    }
    return 0;
...
function resetSort(idTable, dataForm) {
  ...
  clearTable(idTable);
  createTable(buildings, idTable); // 'originalData' - это массив исходных данных таблицы
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  searchButton.addEventListener('click', function() {
    ...
    // Вызываем функцию filterTable с текущими данными, id таблицы и формой
